...
public class FlowFieldView extends Widget {
    ...
    public FlowFieldView(
            int x,
            int y,
            int width,
            int height,
            FlowFieldControls controls) {
    ...
    private void BuildParticles(int amount) {
        ...
        for (int i = 0; i < _controls.ParticleAmount; i++) {
            particles[i] = new Particle(
                    new Vec2f(random.range(0, Width), random.range(0, Height)),
                    new Vec2f(random.nextFloat(), random.nextFloat()),
                    random.range(3, 5),
    ...
    private class Particle {
        ...
        public void Run() {
            ...
            this.color = Kernel.Display.Rgb(
                    (int) (fac_x * 255),
                    (int) (150 * avg), (int) (fac_y * 255));

        }

        @SJC.Inline
        public void Move() {
            float angle = noise.noise(
                    this.loc.x / _controls.NoiseScale,
                    this.loc.y / _controls.NoiseScale,
                    (frameCount * _controls.NoiseChangeSpeed) / _controls.NoiseScale)
                    * Math.TWO_PI * _controls.NoiseStrength;
    ...
    static class PerlinNoise {
        private static final int[] Permutation = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95,
                ...
                196, 135, 130, 116, 188, 159, 86, 164, 100, 109, 198, 173, 186,
                3, 64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118, 126,
                255, 82, 85, 212, 207, 206, 59, 227, 47, 16, 58, 17, 182, 189, 28,
                42, 223, 183, 170, 213, 119, 248, 152, 2, 44, 154, 163, 70, 221,
