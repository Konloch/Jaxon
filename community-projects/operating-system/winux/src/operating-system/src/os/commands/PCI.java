...
public class PCI extends CommandTask
{
	...
	{
		MAGIC.wIOs32(PCI_ADDRESS, 0x80000000 | (busAddress << 16) | (devNumber << 11) | (funcAddress << 8) | (register << 2));
	}
	...
	private final int rowColors = (Color.mix(Color.GRAY | Color.BRIGHT, Color.BLUE) << 8) | Color.mix(Color.GRAY | Color.BRIGHT, Color.BLUE | Color.BRIGHT);
	...
	public static void printPCI(Terminal out)
	{
		...
		while (d != null)
		{
			...
			templ.start(out);
			templ.p(d.bus).p(d.device).p(d.function).p(d.baseClassCode).p(PCIController.getBaseClassCodeDescription(d.baseClassCode)).p(d.subClassCode).p(d.vendorID).p(d.deviceID);
			d = generator.next();
		}
