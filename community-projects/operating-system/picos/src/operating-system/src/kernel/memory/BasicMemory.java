...
public class BasicMemory extends MemIntf
{
	...
	public static void init()
	{
		...
		nextFreeAddress = (MAGIC.imageBase + (MAGIC.rMem32(MAGIC.imageBase + KernelConst.KM_IMAGESIZEREL) + 0xFFF)) & ~0xFFF;
		//get an instance of BasicMemory allocated with BasicMemory :)
		Kernel.mem = (MemIntf) basicAllocate(MAGIC.getInstScalarSize("BasicMemory"), MAGIC.getInstRelocEntries("BasicMemory"), MAGIC.clssDesc("BasicMemory"));
	}
	...
	public static Object basicAllocate(int scalarSize, int relocEntries, SClassDesc type)
	{
		...
		if (lastObjectAddress != 0)
			MAGIC.assign(MAGIC.cast2Obj(lastObjectAddress)._r_next, me); //enter next-object in last object
		//remember the work
