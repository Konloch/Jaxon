...
public class Kernel
{
	...
	public static void main()
	{
		...
		directPrintChar('N', 1, 0, 0x07);
		BasicMemory.init();
		directPrintChar('I', 1, 0, 0x07);
		Interrupts.init();
		directPrintChar('D', 1, 0, 0x07);
		DeviceList.initDevices();
		directPrintChar('E', 1, 0, 0x07);
		ints.sti();
		directPrintChar('S', 1, 0, 0x07);
		Scheduler.run();
	}
	
	public static void directPrintInt(int val, int base, int len, int x, int y, int col, boolean leadingZero)
	{
		int i, addr, dignum, digchr;
		long uval, ubase;
		
		if (len < 1 || base < 2 || base > 16 || x < 0 || x + len > 80 || y < 0 || y > 24)
	...
	public static void setMem32(int addr, int cnt, int val)
	{
		...
		MAGIC.inlineOffset(1, addr); //mov edi,[e/rbp+16/32]
		MAGIC.inline(0x8B, 0x45);
		MAGIC.inlineOffset(1, val); //mov eax,[e/rbp+8/16]
