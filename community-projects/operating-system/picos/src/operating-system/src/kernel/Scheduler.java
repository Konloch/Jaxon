...
public class Scheduler
{
	...
	{
		DeviceList.out.cls();
		DeviceList.out.print("Welcome to Operating-System running in ");
	...
	private static boolean initVesa()
	{
		if (DeviceList.vesa != null)
		{
			...
			DeviceList.out.println("VESA not supported");
		return false;
	}
	...
	private static void printInput()
	{
		...
		while (DeviceList.in.eventAvail())
		{
			...
			if ((modifier & (KEvT.MD_ANY_CTRL | KEvT.MD_ANY_ALT | KEvT.MD_ANY_SHIFT)) == (short) 0)
			{ //system key
	...
	{
		return ((((l >>> 16) * (xRes - x) + (r >>> 16) * x) / xRes) << 16) | (((((l >>> 8) & 0xFF) * (xRes - x) + ((r >>> 8) & 0xFF) * x) / xRes) << 8) | (((l & 0xFF) * (xRes - x) + (r & 0xFF) * x) / xRes);
	}
