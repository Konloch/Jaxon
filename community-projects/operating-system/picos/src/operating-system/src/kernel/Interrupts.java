...
public class Interrupts
{
	...
	public static void createTable()
	{
		...
		destWOE = MAGIC.rMem32(cls + MAGIC.mthdOff("Interrupts", "firstLevelHandlerWOE")) + MAGIC.getCodeOff();
		destWEC = MAGIC.rMem32(cls + MAGIC.mthdOff("Interrupts", "firstLevelHandlerWEC")) + MAGIC.getCodeOff();
		//create entries for exceptions
		while (i <= 0x07)
			createEntry(i++, destWOE);
		while (i <= 0x0E)
			createEntry(i++, destWEC);
		while (i <= 0x10)
			createEntry(i++, destWOE);
		createEntry(i++, destWEC);
		while (i < KernelConst.KC_INTCOUNT)
			createEntry(i++, destWOE); //also create entries for hardware interrupts
		//create new exc-table and copy old entries if existing
		newHandler = new ExcHandler[0x20];
		if (handler != null)
			for (i = 0; i < 0x20; i++)
				newHandler[i] = handler[i];
		handler = newHandler;
		...
		if (devList != null)
			for (i = 0; i < KernelConst.KC_INTCOUNT - 0x20 && i < devList.length; i++)
	...
	private static void createEntry(int no, int dest)
	{
		...
		MAGIC.wMem64(KernelConst.KM_GDTADDR + (dstSel << 3), MAGIC.rMem64(KernelConst.KM_GDTADDR + (1 << 3)));
	}
	...
	protected static void firstLevelHandlerWOE()
	{
		...
		if (MAGIC.ptrSize == 4)
		{ //ia32
			MAGIC.inline(0x8C, 0x4D);
			MAGIC.inlineOffset(1, cs); //mov [ebp-0x04],cs
			MAGIC.inline(0x8B, 0x3D);
			MAGIC.inline32(KernelConst.KM_INTDESC); //mov edi,[KernelConst.KM_INTDESC]
		}
		else
		{ //amd64
			MAGIC.inline(0x8C, 0x4D);
			MAGIC.inlineOffset(1, cs); //mov [ebp-0x08],cs
			MAGIC.inline(0x8B, 0x3C, 0x25);
			MAGIC.inline32(KernelConst.KM_INTDESC); //mov edi,[KernelConst.KM_INTDESC]
		}
		...
		if (no >= handler.length)
		{
			no -= handler.length;
			if (devList[no] != null)
				devList[no].handleIRQ(no);
			DeviceList.irq.ackIRQ(no);
		}
		else if (handler[no] != null)
			handler[no].handle(no, false, 0);
	}
