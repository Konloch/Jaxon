...
public class Keyboard extends Device
{
	...
	private void toggleModifierAndRefreshLEDs(short mod)
	{
		byte LEDcode = (byte) 0;
		modifier ^= mod;
		if ((modifier & SCROLL_LOCK) != (short) 0)
			LEDcode |= (byte) 1;
		if ((modifier & NUM_LOCK) != (short) 0)
			LEDcode |= (byte) 2;
		if ((modifier & CAPS_LOCK) != (short) 0)
			LEDcode |= (byte) 4;
		
		/* wait */
		
		while (((int) MAGIC.rIOs8(0x64) & 0x2) != 0);
	...
	private boolean isRegularModifier(int code)
	{
		...
		{
			case 0x1D:
				setModifier(LEFT_CTRL, make);
				break;
			case 0xE01D:
				setModifier(RIGHT_CTRL, make);
				break;
			case 0x38:
				setModifier(LEFT_ALT, make);
				break;
			case 0xE038:
				setModifier(RIGHT_ALT, make);
				break;
			case 0x2A:
				setModifier(LEFT_SHIFT, make);
				break;
			case 0xE02A:
				setModifier(RIGHT_SHIFT, make);
				break;
			case 0xE05B:
				setModifier(LEFT_WIN, make);
				break;
			case 0xE05C:
				setModifier(RIGHT_WIN, make);
				break;
			case 0xE05D:
				setModifier(CONTEXT, make);
				break;
			case 0x3A:
	...
	{
		return (modifier & (LEFT_CTRL | RIGHT_CTRL)) != (short) 0 && (modifier & (LEFT_SHIFT | RIGHT_SHIFT)) != (short) 0 && code == 0x01;
	}
}
