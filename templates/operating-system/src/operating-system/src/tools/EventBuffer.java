...
public class EventBuffer extends RingBuffer
{
	...
	public boolean insertEvent(byte HIDType, byte HIDNr, short code, int codeExt)
	{
		...
		// 15.. 8: devID
		//  7.. 0: devType
		if ((HIDType == (byte) -1) || (HIDNr == (byte) -1))
			return false; //no valid HID
		//return false if no space left, true if successfull insertion
		return putLong((((long) HIDType) & 0xFFl) | ((((long) HIDNr) & 0xFFl) << 8) | ((((long) code) & 0xFFFFl) << 16) | ((((long) codeExt) & 0xFFFFFFFFl) << 32));
	}
