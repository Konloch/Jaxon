...
public class KEvT
{
	...
	private final static int codeVal[] = {0x47, 0x48, 0x49, 0x4B, 0x4C, 0x4D, 0x4F, 0x50, 0x51, 0x52, 0x53, 0, //end NUM-block with 0
			0x1D, 0x2A, 0x36, 0x38, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x54, 0x57, 0x58, 0xE01D, 0xE037, 0xE038, 0xE046, 0xE047, 0xE048, 0xE049, 0xE04B, 0xE04D, 0xE04F, 0xE050, 0xE051, 0xE052, 0xE053, 0xE05B, 0xE05C, 0xE05D, 0xE11D45};
	private final static char transVal[] = {KC_HOME, KC_CURSUP, KC_PAGEUP, KC_CURSLEFT, KC_CENTER, KC_CURSRIGHT, KC_END, KC_CURSDOWN, KC_PAGEDOWN, KC_INS, KC_DEL, '\u0000', //end num-block for index-matching
			KC_LCTRL, KC_LSHIFT, KC_RSHIFT, KC_LALT, KC_CAPSLOCK, KC_F1, KC_F2, KC_F3, KC_F4, KC_F5, KC_F6, KC_F7, KC_F8, KC_F9, KC_F10, KC_NUM, KC_SCROLL, KC_SYSRQ, KC_F11, KC_F12, KC_RCTRL, KC_PRINT, KC_RALT, KC_BREAK, KC_HOME, KC_CURSUP, KC_PAGEUP, KC_CURSLEFT, KC_CURSRIGHT, KC_END, KC_CURSDOWN, KC_PAGEDOWN, KC_INS, KC_DEL, KC_LWIN, KC_RWIN, KC_CONTEXT, KC_PAUSE};
	...
	public boolean useTable(int country)
	{
		switch (country)
		{
			...
			case 49:
				translation = ByteData.DEKeyTable;
				return true;
		}
		return false;
	}
	...
	public char getChar(short code, int codeExt)
	{
		...
		if (translation == null)
			return (char) 0x0000; //not initialized
		...
		if (codeExt > 0xFFFF //unsupported 3-byte-key-codes (ie.: pause)
				...
				|| codeExt >= TRANSCODES)
			return (char) 0x0000; //unsupported make-code
		
		if ((code & MD_ANY_SHIFT) != (short) 0)
			shift = true;
		if ((code & MD_CAPS_LOCK) != (short) 0)
			shift = !shift;
		if ((code & MD_ANY_CTRL) != (short) 0)
			ctrl = true;
		if ((code & MD_LEFT_ALT) != (short) 0)
			alt = true;
		if ((code & MD_RIGHT_ALT) != (short) 0)
			altgr = true;
		
		if (ctrl && alt)
		{ //ctrl+alt should emulate altgr
			altgr = true;
			ctrl = false;
			alt = false;
		}
		...
		if ((ctrl || alt) //no regular key
				|| (altgr && shift))
			return (char) 0x0000; //invalid modifier-combination for regular key
		
		if (shift)
			return (char) ((int) translation[codeExt + TRANSCODES] & 0xFF); //shift char
		if (altgr)
			return (char) ((int) translation[codeExt + TRANSCODES * 2] & 0xFF); //altgr char
		if ((code & MD_NUM_LOCK) == (short) 0 && codeExt >= 0x47 && codeExt <= 0x53 //these keys are not normal characters and have to be taken with getKey(.)
				&& codeExt != 0x4A && codeExt != 0x4E)
			return (char) 0x0000; //keys in between are independent of NUM-state and correctly assigned in layer0-array
	...
	public char getKey(short code, int codeExt)
	{
		...
		if ((codeExt & 0x80) != 0)
			return (char) 0x0000; //break-code -> no character
		if ((code & MD_NUM_LOCK) == (short) 0)
		{ //scan for NUM-off-chars
			while (codeVal[ind] != 0)
			{
				if (codeVal[ind] == codeExt)
					return transVal[ind];
				ind++;
			}
		}
			...
			while (codeVal[ind++] != 0)
				; //step over NUM-off-chars
		if (codeExt <= 0x7F && translation != null && (c = (char) ((int) translation[codeExt] & 0xFF)) != (char) 0x0000)
			return c; //a normal character-key
		while (ind < codeVal.length)
		{ //search through translation-table
			if (codeVal[ind] == codeExt)
				return transVal[ind];
			ind++;
		}
