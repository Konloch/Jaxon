...
public class Surface
{
	 ...
	 * automatically.
	 *
	 * @param float rval red
	 * @param float gval green
	 * @param float bval blue
	 * @param float a ambient coefficient
	 * @param float d diffuse coefficient
	 * @param float s specular coefficient
	 * @param float n shininess
	 * @param float r reflectivity
	 * @param float t transparency
	 * @param float nt refractive index
	 */
	 ...
	 *
	 * @param Vector3f p the point of intersection
	 * @param Vector3f n the unit-length surface normal
	 * @param Vector3f v the unit-length vector towards the ray origin
	 * @param Scene    lights, objects, color
	 * @return RGBfCol the color of the surface at the intersection pt
	 */
	public long Shade(Vector3f t1, Vector3f t2, Vector3f t3, Scene scene, Ray tempRay)
	{
		...
		while (lightSources != null)
		{
			...
			else
			{
				...
				if (tempRay.cast(scene.objects) == false)
				{ // direct ray to light
					...
					if (lambert > 0.0f)
					{
						...
						{
							lambert *= 2.0f;
							t3.init(lambert * nx - t1.x, lambert * ny - t1.y, lambert * nz - t1.z);
