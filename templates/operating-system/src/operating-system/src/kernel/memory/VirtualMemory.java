...
public class VirtualMemory extends MemIntf
{
	...
	//BasicMemory is installed already, new is working
	public static void init()
	{
		...
		{ //protected mode
			...
			for (i = 0; i < 1024; i++)
				MAGIC.wMem32((i << 2) + KernelConst.KM_PAGEDIR, ((i << 12) + pagetables) | 3);
			...
			for (i = 0; i < 1024 * 1024 - 1; i++)
				MAGIC.wMem32((i << 2) + pagetables, (i << 12) | 0x3);
			MAGIC.wMem32(1024 * 1024 - 1 + pagetables, 0);
			//set pagedirectory register cr3
			MAGIC.inline(0xB8);
			MAGIC.inline32(KernelConst.KM_PAGEDIR); //mov eax,KernelConst.KM_PAGEDIR
		...
		{ //long mode
			...
			for (i = 0; i < 4; i++)
				MAGIC.wMem32(KernelConst.KM_PAGEDIR + 4096 + (i << 3), ((i << 12) + pagetables) | 3);
			//create level three page directory as 1:1-mapping for 0-4 GB except last page (for tests)
			for (i = 0; i < 4 * 512 - 1; i++)
				MAGIC.wMem32((i << 3) + pagetables, (i << 21) | 0x83);
			//set pagedirectory register cr3
			MAGIC.inline(0xB8);
			MAGIC.inline32(KernelConst.KM_PAGEDIR); //mov eax,KernelConst.KM_PAGEDIR
